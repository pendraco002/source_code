  ...
  async fetch(request, env2, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/sessions" && request.method === "POST") {
        return await saveSession(request, env2, userId);
      } else if (url.pathname === "/sessions" && request.method === "GET") {
        return await loadSessions(env2, userId);
      } else if (url.pathname.match(/^\/sessions\/[^\/]+$/) && request.method === "GET") {
        const sessionId = url.pathname.split("/")[2];
        return await loadSession(env2, userId, sessionId);
      } else if (url.pathname.match(/^\/sessions\/[^\/]+$/) && request.method === "PUT") {
        const sessionId = url.pathname.split("/")[2];
        return await updateSession(request, env2, userId, sessionId);
      } else if (url.pathname.match(/^\/sessions\/[^\/]+$/) && request.method === "DELETE") {
        const sessionId = url.pathname.split("/")[2];
        return await deleteSession(env2, userId, sessionId);
      }
...
async function saveSession(request, env2, userId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
  ...
  try {
    ...
    console.error("Save session error:", error3);
...
async function loadSessions(env2, userId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Content-Type": "application/json"
  };
  try {
    ...
    console.error("Load sessions error:", error3);
...
__name(loadSessions, "loadSessions");
async function loadSession(env2, userId, sessionId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Content-Type": "application/json"
  };
  try {
    ...
    console.error("Load session error:", error3);
...
async function updateSession(request, env2, userId, sessionId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Content-Type": "application/json"
  };
  try {
    ...
    console.error("Update session error:", error3);
...
async function deleteSession(env2, userId, sessionId) {
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Content-Type": "application/json"
  };
  try {
    ...
    console.error("Delete session error:", error3);
